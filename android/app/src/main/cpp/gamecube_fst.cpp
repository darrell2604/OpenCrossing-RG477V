...
bool GameCubeFst::parse(const std::vector<std::uint8_t>& fst_bytes, std::string* error) {
    ...
    if (node_count == 0 || node_count > (std::numeric_limits<std::size_t>::max() / kNodeSize)) {
        set_error(error, "FST node count is invalid");
        return false;
    }

    const std::size_t node_bytes = static_cast<std::size_t>(node_count) * kNodeSize;
    if (node_bytes > fst_bytes.size()) {
    ...
    struct DirectoryFrame { std::string path; std::uint32_t end_index; };
    ...
    for (std::uint32_t index = 1; index < node_count; ++index) {
        while (stack.size() > 1 && index >= stack.back().end_index) stack.pop_back();
        ...
        if (name_pos >= fst_bytes.size()) {
            set_error(error, "FST name offset is out of bounds");
            entries_.clear();
            return false;
        }
        ...
        if (std::memchr(name, '\0', max_name) == nullptr) {
            set_error(error, "FST name is not terminated");
            entries_.clear();
            return false;
        }
...
bool GameCubeFst::read_file(std::string_view path,
    ...
    if (!output) { set_error(error, "Output buffer is null"); return false; }
    output->clear();
    const GameCubeFstEntry* entry = find(path);
    if (!entry) { set_error(error, "FST path not found"); return false; }
    if (entry->directory) { set_error(error, "FST path is a directory"); return false; }
    const std::uint64_t end = static_cast<std::uint64_t>(entry->offset) + entry->size;
    if (end > disc_image.size()) { set_error(error, "FST file extends beyond disc image"); return false; }
